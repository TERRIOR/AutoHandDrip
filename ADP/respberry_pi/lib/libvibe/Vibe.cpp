...
void ViBe_BGS::processFirstFrame(const Mat _image)
{
	...
	for (int i = 0; i < _image.rows; i++)
	{
		for (int j = 0; j < _image.cols; j++)
		{
			...
			{
				// Random pick up NUM_SAMPLES pixel in neighbourhood to construct the model   随机在四周选取NUM_SAMPLES个像素去构建模型
	...
	for (int i = 0; i < _image.rows; i++)
	{
		for (int j = 0; j < _image.cols; j++)
		{
			...
			while (matches < MIN_MATCHES && count < NUM_SAMPLES)
			{
				...
				if (dist < RADIUS)//差值小于RADIUS 就加一？越大越容易加一  则越容易作为背景
				matches++;
				count++;
			}
			//如果 是因为matches >MIN_MATCHES而退出循环，则下面就要执行   作为背景 // 如果由于所有采样值遍历完而退出的就不需要执行  作为前景
			//因此MIN_MATCHES越大 越难作为背景
